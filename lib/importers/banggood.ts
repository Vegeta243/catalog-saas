...
export async function importFromBanggood(url: string): Promise<ImportResult> {
  ...
  if (!ok) return { success: false, url, error: 'Banggood inaccessible' }
  ...
  return {
    success: true, url,
    product: {
      title: og.title?.replace(/\s*[-|]\s*Banggood.*/i, '').trim() || 'Produit Banggood',
      ...
      vendor: 'Banggood', platform: 'banggood', sourceUrl: url,
    }
  }
}
